...
from .github_client import GitHubClient, PullRequest, WorkflowRun, CheckRun
from .config import PraierConfig, GitHubConfig
...
class PRMonitor:
    ...
    def __init__(self, config: PraierConfig):
        self.config = config
        self.clients = {}
        self.pr_states: Dict[str, Dict[int, PRState]] = {}  # server_name -> pr_number -> state
        
        # Initialize GitHub clients
        for github_config in config.github_servers:
            if not github_config.token:
                logger.warning(f"No token provided for GitHub server '{github_config.name}', skipping")
                continue
    ...
    async def monitor_cycle(self):
        ...
        for server_name, server_info in self.clients.items():
            task = asyncio.create_task(
                self.monitor_server(server_name, server_info)
            )
            tasks.append(task)
    ...
    async def monitor_server(self, server_name: str, server_info: Dict):
        ...
        if not repositories:
            logger.warning(f"No repositories configured for monitoring on {server_name}")
            return
        ...
        for repository in repositories:
            try:
                ...
                logger.error(f"Error monitoring repository {repository} on {server_name}: {e}")
    
    async def monitor_repository(self, server_name: str, client: GitHubClient, repository: str):
    ...
    async def process_pull_request(self, server_name: str, client: GitHubClient, repository: str, pr: PullRequest):
    ...
    def get_monitoring_stats(self) -> Dict:
        ...
        stats = {
            ...
            'repositories': self.config.monitoring.repositories.copy() if self.config.monitoring.repositories else []
        }
    ...
    def cleanup_stale_prs(self, max_age_hours: int = 24):
        ...
        for server_name, pr_states in self.pr_states.items():
            stale_prs = [
                pr_num for pr_num, state in pr_states.items()
                if state.last_updated < cutoff_time
            ]
            
            for pr_num in stale_prs:
                del pr_states[pr_num]
                logger.debug(f"Cleaned up stale PR state for #{pr_num} on {server_name}")