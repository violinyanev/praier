...
def monitor(ctx):
    ...
    if not any(server.token for server in config.github_servers):
        click.echo("Error: No GitHub tokens configured. Set GITHUB_TOKEN environment variable or provide a config file.", err=True)
        sys.exit(1)
    
    if not config.monitoring.repositories:
        click.echo("Warning: No repositories configured for monitoring. Set PRAIER_REPOSITORIES environment variable.", err=True)
...
def status(ctx):
    ...
    click.echo(f"  Max concurrent requests: {config.monitoring.max_concurrent_requests}")
    click.echo(f"  Auto-approve actions: {config.monitoring.auto_approve_actions}")
    click.echo(f"  Auto-fix with Copilot: {config.monitoring.auto_fix_with_copilot}")
