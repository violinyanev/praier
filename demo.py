...
class MockGitHubClient:
    ...
    def request_copilot_fix(self, repository, pr_number, failing_checks):
        ...
        logger.info(f"🤖 Requested GitHub Copilot fix for PR #{pr_number} in {repository}")
    ...
    for cycle in range(1, 4):
        ...
        try:
            ...
            if mock_client.copilot_requests:
                print(f"   🤖 Requested Copilot fixes for {len(mock_client.copilot_requests)} PRs")
            
            # Get stats
            stats = monitor.get_monitoring_stats()
            print(f"   📈 Stats: {stats['total_prs']} PRs tracked across {stats['servers']} servers")
