...
class TestInstallScript:
    ...
    def test_install_script_security_checks(self):
        ...
        assert "EUID" in content or "$(id -u)" in content, "Script should check for root privileges"
...
class TestDemoScript:
    ...
    async def test_demo_script_mock_functionality(self, mock_pr_monitor):
        ...
        from demo import MockGitHubClient, demo_monitoring
        ...
        assert mock_client.approve_workflow_run("test/repo", "run-123") == True
        assert "run-123" in mock_client.approved_runs
        
        assert mock_client.request_copilot_fix("test/repo", 123, check_runs) == True
...
class TestScriptIntegration:
    ...
    def test_praier_command_available(self):
        ...
        entry_points = list(pkg_resources.iter_entry_points('console_scripts', 'praier'))
