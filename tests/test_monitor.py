...
class TestPRState:
    ...
    def test_pr_state_creation(self):
        ...
        assert state.copilot_requested is False
...
class TestPRMonitor:
    ...
    def setup_method(self):
        ...
        self.config = PraierConfig(
            github_servers=[
                GitHubConfig(
                    name="test", url="https://api.github.com", token="test-token"
                )
            ],
    ...
    async def test_monitor_repository(self, mock_github_client):
        ...
        mock_client_instance.get_pull_requests.assert_called_once_with(
            "owner/repo1", state="open"
        )

        # Verify process_pull_request was called for each PR
        monitor.process_pull_request.assert_called_once_with(
            "test", mock_client_instance, "owner/repo1", mock_pr
        )
    ...
    async def test_process_pull_request_new_pr(self, mock_github_client):
        ...
        await monitor.process_pull_request(
            "test", mock_client_instance, "owner/repo1", mock_pr
        )
    ...
    async def test_process_workflow_runs_auto_approve(self, mock_github_client):
        ...
        mock_client_instance.approve_workflow_run.assert_called_once_with(
            "owner/repo1", "run-123"
        )
    ...
    async def test_process_check_runs_copilot_request(self, mock_github_client):
        ...
        mock_client_instance.request_copilot_fix.assert_called_once_with(
            "owner/repo1", 123, [mock_check_run]
        )

        # Verify flag was set
        assert pr_state.copilot_requested is True
    ...
    async def test_copilot_request_reset_on_sha_change(self, mock_github_client):
        ...
        assert pr_state.copilot_requested is False
