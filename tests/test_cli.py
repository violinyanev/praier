...
class TestCLI:
    ...
    def test_monitor_command_success(self, mock_asyncio_run, mock_pr_monitor):
        ...
        env = {"GITHUB_TOKEN": "test-token", "PRAIER_REPOSITORIES": "owner/repo1"}
    ...
    @patch("praier.cli.PRMonitor")
    @patch("asyncio.run")
    def test_monitor_command_keyboard_interrupt(
        self, mock_asyncio_run, mock_pr_monitor
    ):
    ...
    def test_test_connection_invalid_server(self):
        ...
        with patch.dict(os.environ, env, clear=True):
            result = self.runner.invoke(
                cli, ["test-connection", "owner/repo", "--server", "nonexistent"]
            )
            assert result.exit_code == 1
